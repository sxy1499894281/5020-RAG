...
## 🧠 创新思路与课堂 Presentation 方案
 ...
 - **[SLA 驱动模式选择]**
  ...
  - **先搞懂几个常见术语**
    - `术语型` 查询：提问时主要堆砌具体词汇或名词，比如“Transformer 架构层数”，系统靠这些关键词去匹配文档。
    - `语义型` 查询：句子更像自然语言、含义更模糊，例如“怎么让大模型更懂长文档”，需要模型理解语境才能匹配。
    - `初排`：检索系统第一次粗排出的 Top-k 结果，速度快但可能不够准确，后面还能再精挑细选。
    - `长尾词`：极少出现的冷门词，比如专有名词或拼写特别的术语，常被传统方法忽略。
    - `Rerank/Cross-Encoder`：把初排出的候选再丢给一个更聪明但更慢的模型逐个评分，确保前排更靠谱。
    - `QE/PRF`：Query Expansion / Pseudo Relevance Feedback，先用原始问题检索一遍，再从命中文档里总结出新词或重写问题，帮助扩大召回面。

  - **动态混合权重：自适应 alpha**
    - 想象有两位“搜索员”：一位擅长查关键词的图书管理员（BM25），另一位擅长理解语义的聪明助理（向量检索）。`alpha` 就是两人的合作比例。
    - 简单场景：问题很短、关键词明显，就让图书管理员主导；问题像聊天一样，就让聪明助理多出力。
    - 做法：先粗略判断问题属于术语型还是语义型，再动态设定两人的“出场顺序”，让最终的 Top-k 不偏不倚。

  - **跨编码器重排（Rerank）**
    - 初排像“快速浏览”，可能把一些相关但不够精准的结果放在前面。
    - 引入 Cross-Encoder 就像请专家逐条阅读候选，把真正贴题的文档放到最前面，保证引用内容的准确性。
    - 缺点是专家阅稿慢，所以只挑 Top-N 条让他看，控制耗时。

  - **查询扩展（QE：PRF + LLM 改写）**
    - 有时用户一句话表达太含糊，或用了系统没见过的冷门说法，导致搜不到关键答案。
    - 解决办法：先用原问题检索一次，从命中的文档里抽取高频词，再请 LLM 生成好几个意思相近的问法。
    - 这些新增问题一起去检索，最后合并结果并去重，就能找到更多之前漏掉的内容。

  - **证据片段高亮（句级）**
    - 用户常抱怨“虽然给了文档，但我不知道答案藏在哪儿”。
    - 句级打分就像在文档里贴便利贴，把最可能回答问题的句子挑出来，高亮给用户看。
    - 既提升说服力，又让上下文更短，减少模型生成时的噪音。

  - **类别感知检索**
    - 文档库里跨学科内容很多，如果提问“量子电路优化”，不希望跑来一堆生物论文。
    - 通过识别问题所属类别（例如利用 arXiv 的分类标签），提前过滤或加权，只保留同领域的候选，提高准确率。

  - **SLA 驱动模式选择**
    - 不同场景对“速度 vs 效果”的需求不同：有的要秒回，有的宁可等几秒也要最优答案。
    - 系统根据当前的延迟预算自动选择策略：预算紧张时只走快速 BM25；预算宽裕就启用混合检索加 rerank。
    - 这样既能满足服务级别协议（SLA），又不会牺牲用户体验。
